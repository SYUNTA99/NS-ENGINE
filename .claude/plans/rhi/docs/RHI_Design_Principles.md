...
// 機能レベルによる分岐
if (GRHIGlobals.RayTracing.Tier >= RayTracingTier::Tier1_1)
{
    // 高度なRT機能使用
}
```
...
struct RHICommandDraw
{
    ...
    static void Execute(RHICommandListBase& CmdList, const RHICommandDraw& Cmd);
};
```
...
```cpp
// 悪い例: 暗黙のリソース状態遷移
void DrawMesh(RHITexture* Texture)
{
...
// 良い例: 明示的なバリア
void DrawMesh(RHICommandList& RHICmdList, RHITexture* Texture)
{
    // 明示的な状態遷移
    RHICmdList.Transition(RHITransitionInfo(
        Texture,
        RHIAccess::CopyDest,      // 前の状態
        RHIAccess::SRVGraphics    // 新しい状態
    ));
...
void UseTexture()
{
    RefCountPtr<RHITexture> Texture = CreateTexture();
...
class DeferredDeletionQueue
{
    ...
    Array<PendingDelete> PendingDeletes;
...
```cpp
// Transient リソースの使用
void RenderFrame(RHICommandList& RHICmdList)
{
    // Transient テクスチャ取得（メモリ再利用）
    RHITransientTexture* ShadowMap = TransientAllocator->CreateTexture(
    ...
    // 同じメモリを別のテクスチャとして再利用
    RHITransientTexture* GBuffer = TransientAllocator->CreateTexture(
...
```cpp
// コマンド記録（メモリ確保のみ）
void RecordCommands(RHICommandList& RHICmdList)
{
    // これらは即座にGPUに送信されない
...
// 実行（RHIスレッドまたはタスク）
void ExecuteCommands(RHICommandListImmediate& RHICmdList)
{
    // ここでGPUにコマンドを送信
    RHICmdList.ImmediateFlush(ImmediateFlushType::FlushRHIThread);
}
```
...
```cpp
// 通常パス: 遅延実行
void DrawDeferred(RHICommandList& RHICmdList)
{
    RHICmdList.Draw(...);  // キューに記録
}

// バイパスパス: 即時実行
void DrawImmediate(RHICommandListImmediate& RHICmdList)
{
    // 即時フラッシュしてから実行
    RHICmdList.ImmediateFlush(ImmediateFlushType::FlushRHIThread);
...
```cpp
// 並列コマンドリスト記録
void RenderSceneParallel(RHICommandListImmediate& RHICmdList)
{
    Array<RHICommandList*> ParallelLists;
    ...
    // メインリストにマージ
    for (RHICommandList* CmdList : ParallelLists)
    {
...
```cpp
void RenderWithAsyncCompute(RHICommandListImmediate& RHICmdList)
{
    // グラフィックス作業開始
    BeginShadowRendering(RHICmdList);

    // 非同期コンピュート開始（並列実行）
    RHIAsyncComputeCommandListImmediate& AsyncCmdList =
        RHICommandListExecutor::GetImmediateAsyncComputeCommandList();
...
```cpp
// 状態追跡構造体
struct RHITrackedResource
{
    RHIAccess CurrentAccess = RHIAccess::Unknown;
    RHIPipeline CurrentPipeline = RHIPipeline::Graphics;
    uint32 SubresourceStates[MaxMipLevels * MaxArraySlices];
};

// 明示的な遷移要求
void TransitionResource(RHICommandList& RHICmdList,
                       RHITexture* Texture,
                       RHIAccess OldAccess,
                       RHIAccess NewAccess)
{
...
// 動的状態は別途設定
void SetDynamicState(RHICommandList& RHICmdList)
{
    // ビューポート（動的）
...
```cpp
// バインドレスの使用
void DrawBindless(RHICommandList& RHICmdList)
{
    // ディスクリプタハンドルを取得
    RHIDescriptorHandle TextureHandle = Texture->GetDefaultBindlessHandle();
...
```cpp
// 悪い例: 個別バリア
void TransitionsIndividual(RHICommandList& RHICmdList)
{
    RHICmdList.Transition(RHITransitionInfo(Texture1, ...));  // バリア1
    RHICmdList.Transition(RHITransitionInfo(Texture2, ...));  // バリア2
    RHICmdList.Transition(RHITransitionInfo(Buffer1, ...));   // バリア3
    // 3回のバリア発行
}

// 良い例: バッチバリア
void TransitionsBatched(RHICommandList& RHICmdList)
{
    Array<RHITransitionInfo> Transitions;
    Transitions.Add(RHITransitionInfo(Texture1, ...));
    Transitions.Add(RHITransitionInfo(Texture2, ...));
    Transitions.Add(RHITransitionInfo(Buffer1, ...));
...
```cpp
void UseSplitBarrier(RHICommandList& RHICmdList)
{
    // バリア開始（遷移を開始するが完了を待たない）
    RHICmdList.BeginTransitions({
        RHITransitionInfo(Texture, RHIAccess::SRVGraphics, RHIAccess::AVCompute)
    });
...
```cpp
// デバッグビルドでの検証
void SetTexture(uint32 Slot, RHITexture* Texture)
{
#if DO_CHECK
...
class ValidationRHI : public DynamicRHI
{
...
void RenderPass(RHICommandList& RHICmdList, const TCHAR* PassName)
{
    // パス開始マーカー
...
public:
    ...
    // プラットフォーム固有拡張
    virtual bool RHISupportsExtension(const Name& ExtensionName)
    {
        return false;
    }

    virtual void* RHIGetExtensionInterface(const Name& ExtensionName)
    {
        return nullptr;
    }
};
...
{
    bool RHISupportsExtension(const Name& ExtensionName) override
    {
...
{
case RayTracingTier::NotSupported:
    // RTなし
    break;
case RayTracingTier::Tier1_0:
    // 基本RT
    break;
case RayTracingTier::Tier1_1:
    // 高度なRT
    break;
}
```
...
```cpp
// 悪い例: 多数のAPI呼び出し
void DrawManyObjects_Slow(RHICommandList& RHICmdList)
{
...
// 良い例: インスタンシング
void DrawManyObjects_Fast(RHICommandList& RHICmdList)
{
...
// さらに良い例: Indirect描画
void DrawManyObjects_Fastest(RHICommandList& RHICmdList)
{
...
| パターン | 用途 | RHIでの適用 |
|----------|------|-------------|
| **コマンド** | 遅延実行 | RHICommand* |
| **ファクトリ** | リソース作成 | DynamicRHI::RHICreate* |
| **ストラテジー** | プラットフォーム切り替え | FD3D12DynamicRHI等 |
| **フライウェイト** | リソース共有 | PSO キャッシュ |
| **RAII** | ライフタイム管理 | RefCountPtr |
| **プール** | 再利用 | RHIRenderQueryPool |
| **バッチ** | 効率化 | RHIBatchedShaderParameters |
