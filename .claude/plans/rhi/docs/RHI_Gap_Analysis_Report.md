...
| 機能 | 現状 | 不足内容 |
|------|------|----------|
| Transient Resource Allocator | Part 6で概念のみ | `RHITransientAllocationFences`詳細、マルチパイプライン同期 |
| VRS (Variable Rate Shading) | Part 9でフラグのみ | `VRSImageDataType`、コンバイナー操作、タイルサイズ設定 |
| バインドレス | Part 3で基本のみ | `RHIBindlessConfiguration`列挙、各モード詳細 |
| レイトレーシング高度機能 | Part 4で基本のみ | クラスター操作、インラインRT、AMD Hit Token |
| リソース遷移 | Part 6で基本のみ | `RHITransitionCreateFlags`、スプリットバリア |
...
API:
  - RHICommitResourceInfo - メモリコミット情報
...
### 3.3 Resource Collections (RHIResourceCollection)
...
クラス:
  - RHIResourceCollection
  - RHIResourceCollectionMember
...
クラス:
  ...
  - UniformBufferStaticBindings

サポートレベル (RHIStaticShaderBindingLayoutSupport):
  - Unsupported     : 非サポート
...
詳細設定:
  ...
  - VRSImageDataType : Palette vs Fractional
  - ImageFormat : VRSイメージフォーマット

コンバイナー操作 (VRSRateCombiner):
  - Passthrough : そのまま使用
  - Override    : 上書き
  - Min         : 最小値選択
  - Max         : 最大値選択
...
構造体:
  - RHIDispatchIndirectParameters
  - RHIDrawIndirectParameters
  - RHIDrawIndexedIndirectParameters
...
### Part 17 で追加された内容
- コマンドリスト実行エンジン (RHICommandListExecutor, RHIThreadMode)
- マルチGPU転送システム (CrossGPUTransferFence, TransferResourceFenceData)
- PSOプリキャッシュシステム (PSOPrecacheRequestID, PSOPrecacheRequestResult, PSORuntimeCreationStats)
- 新GPUプロファイラー (UE::RHI::GPUProfiler namespace)
- 描画統計カテゴリ (RHIDrawStatsCategory, MAX_DRAWCALL_CATEGORY)
- バウンドシェーダーキャッシュ (BoundShaderStateKey, FCachedBoundShaderStateLink_Threadsafe)
- データドリブンプラットフォーム情報 (RHIGlobal, bSupportsRayTracingClusterOps等)
- バリデーション層詳細 (ValidationRHI, RHI_VALIDATION_CHECK)
- リソースユーティリティ (UE::RHIResourceUtils namespace)
- 内部定数・アライメント (SHADER_PARAMETER_STRUCT_ALIGNMENT等)
- 入力レイテンシ追跡 (GInputLatencyTime)
- GPUベンダー情報 (RHIGlobals::GpuInfo)

### Part 16 で追加された内容
- RHIインターフェース列挙型 (RHIInterfaceType, RHIFeatureSupport)
- シェーダー関連列挙型 (ShaderFrequency, RHIShaderBundleMode)
- デスクリプタシステム型 (RHIDescriptorHeapType, RHIDescriptorType, RHIDescriptorHandle)
- バインドレス設定 (RHIBindlessConfiguration)
- リソース遷移フラグ (RHITransitionCreateFlags, ResourceTransitionFlags, RHIAccess)
- パイプライン/コンテキスト (RHIPipeline, RHIThreadMode, RHISubmitFlags)
- サンプラー設定 (SamplerFilter, SamplerAddressMode)
- リソース初期化 (RHIBufferInitAction, RHITextureInitAction, RHITexturePlane)
- Transientリソース型 (RHITransientResourceType, RHITransientAllocationType)
- カラースペース/HDR (ColorSpaceAndEOTF)
- マルチGPU (RHIGPUMask, RHIScopedGPUMask)
- 重要な定数 (MAX_TEXTURE_MIP_COUNT, MaxSimultaneousRenderTargets等)

### Part 15 で追加された内容
- コマンドリストスコープユーティリティ (RHICommandListScopedFence, RHICommandListScopedPipelineGuard)
- バッチシェーダーパラメータシステム (RHIBatchedShaderParameters, RHIBatchedShaderParametersAllocator)
- Shader Bundle ディスパッチ (RHIShaderBundleComputeDispatch, RHIShaderBundleGraphicsDispatch)
- タイムスタンプキャリブレーション (RHITimestampCalibrationQuery)
- クエリプールシステム (RHIRenderQueryPool, RHIPooledRenderQuery)
- テクスチャリファレンス (RHITextureReference)
- イミュータブルサンプラー (ImmutableSamplerState)
- リソースリプレースシステム (RHIResourceReplaceBatcher)
- ビューキャッシュ (RHITextureViewCache, RHIBufferViewCache)
- イベント/デリゲート (RHIPanicEvent, PipelineStateLoggedEvent)
...
### Part 12 で追加された内容
- GPU デフラグアロケータ (GPUDefragAllocator)
- Pipeline File Cache (PipelineFileCacheManager)
- GPU Readback (RHIGPUMemoryReadback)
- Diagnostic Buffer (RHIDiagnosticBuffer)
- Descriptor Allocator (RHIDescriptorAllocator)
- GPU Profiler 新システム
- Resource Replace (RHIResourceReplaceBatcher)
- Lock Tracker (RHILockTracker)
- Shader Binding Layout (RHIShaderBindingLayout)
- PSO LRU Cache (PsoLruCache)
