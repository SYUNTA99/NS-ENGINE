...
複数シェーダーを含むライブラリとシェーダーバリエーション管理を定義する。
...
新規作成:
- `Source/Engine/RHI/Public/IRHIShaderLibrary.h`
...
{
    /// シェーダーライブラリ記述
    /// DXILライブラリ（レイトレーシング用（やユニバーサルシェーダー用
    ...
    public:
        ...
        /// エクスポートからシェーダー作成
        virtual IRHIShader* CreateShaderFromExport(
            const char* exportName,
            EShaderFrequency frequency) = 0;
    };
...
- [ ] RHIShaderLibraryDesc 構造体
- [ ] IRHIShaderLibrary インターフェース
...
namespace NS::RHI
{
    ...
    struct RHI_API RHIPermutationKey
    {
        ...
        void SetBit(uint32 index, bool value) {
            if (value) bits |= (1ull << index);
            else bits &= ~(1ull << index);
        }
...
- [ ] RHIPermutationKey 構造体
- [ ] RHIPermutationDimension 構造体
...
namespace NS::RHI
{
    ...
    public:
        ...
        /// 初期化
        /// @param dimensions パーミュテーション次元の分
        /// @param dimensionCount 次元数
        void Initialize(
        ...
        /// 次元取得
        const RHIPermutationDimension& GetDimension(uint32 index) const {
        ...
        class KeyBuilder {
        public:
            KeyBuilder(const RHIShaderPermutationSet* set) : m_set(set) {}
...
namespace NS::RHI
{
    ...
    public:
        ...
        /// パーミュテーションセット読み込み
        RHIShaderPermutationSet* LoadPermutationSet(
            const char* basePath,
            const RHIPermutationDimension* dimensions,
            uint32 dimensionCount);
...
- [ ] RHIShaderManager クラス
- [ ] LoadShader / LoadShaderLibrary
- [ ] ホットリロード機能
...
namespace NS::RHI
{
    ...
    public:
        ...
        using ProgressCallback = std::function<void(
            uint32 current, uint32 total, const char* currentFile)>;
...
- [ ] RHIPrecompileOptions 構造体
- [ ] RHIPrecompileResult 構造体
- [ ] RHIShaderPrecompiler クラス
