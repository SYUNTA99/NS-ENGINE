...
- `common/utility/types.h` - 基本型（`int32`, `uint64`, `double`）
...
- [ ] `GenericPlatformTime` インターフェース宣言
- [ ] 高精度タイマー関数宣言
- [ ] システム時刻取得関数宣言
- [ ] 時刻変換ユーティリティ
...
namespace NS
{
    ...
    /// └───────────────────────────────────────────┘
    ///
    /// 計算式: Seconds() ≒ Cycles64() × GetSecondsPerCycle64()
    /// ```
    ///
    /// ## 使用パターン
    ///
    ...
    struct GenericPlatformTime
    {
        ...
        /// @note FORCEINLINEだがわずかなオーバーヘッドあり
        ...
        /// @note 例: 3GHz CPUでは約 3.33e-10
        static FORCEINLINE double GetSecondsPerCycle64();
        ...
        {
            return cycles * GetSecondsPerCycle64();
        }
...
- `Seconds()` が単調増加する
- `Cycles64()` が単調増加する
- `CyclesToSeconds(Cycles64())` ≒ `Seconds()` （誤差1ms以内）
- `GetLocalTime()` と `GetUtcTime()` が妥当な値を返す

## 注意事項

- **InitTiming()を呼ばずにSeconds()を使うと未定義動作**
- Cycles64()はCPU間で同期されていない可能性（マルチソケット環境）
- システム時刻はNTP等で変更される可能性があるため、経過時間計測には使わない
