...
- [ ] `ProgramCounterSymbolInfo` 構造体定義
- [ ] `GenericPlatformStackWalk` インターフェース宣言
- [ ] 定数定義（最大スタック深度）
...
namespace NS
{
    ...
    struct GenericPlatformStackWalk
    {
        ...
        ///
        /// @note 失敗時でもoutInfoは初期化され、programCounterは設定される
        /// @note 初回呼び出しはシンボルロードで遅い
        ...
        /// @return 解決成功した数
        ///
        /// @note バッチ処理で個別呼び出しより効率的
        static int32 ProgramCountersToSymbolInfos(
        ...
        /// 現在のスタックトレースをログ出力
        ///
        /// @param skipCount スキップするフレーム数
        /// @note デバッグ用ユーティリティ
        static void DumpStackTrace(int32 skipCount = 0);
...
- ヘッダがコンパイル可能
- `ProgramCounterSymbolInfo` のデフォルト初期化で全フィールドがゼロ/空
- `SafeStrCopy` が長い文字列を切り詰めてヌル終端
- `IsResolved()` がシンボル有無を正しく判定

## 注意事項

- **シンボル解決にはPDBファイルが必要**（Windows）
- 初回のシンボル解決は数百ms〜数秒かかる可能性
- Release最適化ビルドでは関数がインライン化されシンボル解決が不正確になる
- スタックウォークは自スレッドのみ（他スレッドのスタック取得は別API）
