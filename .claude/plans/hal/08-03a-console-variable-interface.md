# 08-03a: ConsoleVariableインターフェース

## 目的

コンソール変数の基底インターフェースと型定義を実装する。

## 参考

[Platform_Abstraction_Layer_Part8.md](docs/Platform_Abstraction_Layer_Part8.md) - セクション2「コンソール変数システム」
UE5.7: `Engine/Source/Runtime/Core/Public/HAL/IConsoleManager.h`

## 依存（commonモジュール）

- `common/utility/types.h` - 基本型（`int32`, `uint32`）

## 依存（HAL）

- 01-04 プラットフォーム型（`TCHAR`）
- 02-03 関数呼び出し規約（`FORCEINLINE`）

## 変更対象

新規作成:
- `source/engine/hal/Public/HAL/IConsoleVariable.h`

## TODO

- [ ] `ConsoleVariableFlags` フラグ列挙型定義
- [ ] `IConsoleObject` 基底インターフェース
- [ ] `IConsoleVariable` 変数インターフェース
- [ ] `IConsoleCommand` コマンドインターフェース
- [ ] `ConsoleVariableDelegate` コールバック型

## 実装内容

```cpp
// IConsoleVariable.h
#pragma once

#include "common/utility/types.h"
#include "HAL/PlatformTypes.h"
#include "HAL/PlatformMacros.h"

namespace NS
{
    /// コンソール変数フラグ
    ///
    /// ビット演算で組み合わせ可能。
    ///
    /// ## フラグの種類
    ///
    /// 1. **属性フラグ** (bit 0-15): 変数の性質を定義
    /// 2. **設定元フラグ** (bit 16-23): 最後に設定した主体を記録
    ///
    /// ## 設定元優先度ルール
    ///
    /// 設定元フラグは**数値が大きいほど優先度が高い**。
    /// 低優先度の設定元は高優先度の設定を上書きできない。
    ///
    /// 優先度順（低→高）:
    /// 1. Constructor（デフォルト値）
    /// 2. Scalability（品質プリセット）
    /// 3. GameSetting（ゲーム内設定UI）
    /// 4. ProjectSetting（エディタ設定）
    /// 5. Commandline（起動引数）
    /// 6. Console（デバッグコンソール）
    /// 7. Code（プログラム直接設定）
    ///
    /// ## 使用例
    ///
    /// ```cpp
    /// // ユーザーがコンソールで設定した値は、
    /// // スケーラビリティ設定で上書きされない
    /// cvar->Set(2, ConsoleVariableFlags::SetByConsole);
    /// cvar->Set(1, ConsoleVariableFlags::SetByScalability);  // 無視される
    /// ```
    enum class ConsoleVariableFlags : uint32
    {
        None            = 0,

        // ========== 属性フラグ (bit 0-15) ==========

        Unregistered    = 1 << 0,   ///< システムに未登録
        // bit 1 reserved
        ReadOnly        = 1 << 2,   ///< 読み取り専用（Codeからのみ変更可）
        Cheat           = 1 << 3,   ///< チート扱い（Shippingビルドで無効）

        RenderThreadSafe = 1 << 4,  ///< レンダースレッドから安全に読み取り可

        Archive         = 1 << 5,   ///< 設定ファイルに保存/読み込み

        ShaderChange    = 1 << 6,   ///< 変更時シェーダ再コンパイルをトリガー

        Scalability     = 1 << 7,   ///< スケーラビリティグループの一部

        Preview         = 1 << 8,   ///< プレビュー用（保存されない）

        // ========== 設定元フラグ (bit 16-23) ==========
        // 数値が大きいほど優先度が高い

        SetByConstructor    = 1 << 16,  ///< コンストラクタで設定（優先度1）
        SetByScalability    = 2 << 16,  ///< スケーラビリティで設定（優先度2）
        SetByGameSetting    = 3 << 16,  ///< ゲーム設定で設定（優先度3）
        SetByProjectSetting = 4 << 16,  ///< プロジェクト設定で設定（優先度4）
        SetByCommandline    = 5 << 16,  ///< コマンドラインで設定（優先度5）
        SetByConsole        = 6 << 16,  ///< コンソールで設定（優先度6）
        SetByCode           = 7 << 16,  ///< コードで設定（優先度7、最高）

        SetByMask       = 0xFF << 16    ///< 設定元マスク
    };

    // ========== フラグ演算子 ==========

    FORCEINLINE ConsoleVariableFlags operator|(ConsoleVariableFlags a, ConsoleVariableFlags b)
    {
        return static_cast<ConsoleVariableFlags>(static_cast<uint32>(a) | static_cast<uint32>(b));
    }

    FORCEINLINE ConsoleVariableFlags operator&(ConsoleVariableFlags a, ConsoleVariableFlags b)
    {
        return static_cast<ConsoleVariableFlags>(static_cast<uint32>(a) & static_cast<uint32>(b));
    }

    FORCEINLINE ConsoleVariableFlags operator~(ConsoleVariableFlags a)
    {
        return static_cast<ConsoleVariableFlags>(~static_cast<uint32>(a));
    }

    FORCEINLINE ConsoleVariableFlags& operator|=(ConsoleVariableFlags& a, ConsoleVariableFlags b)
    {
        a = a | b;
        return a;
    }

    FORCEINLINE bool HasFlag(ConsoleVariableFlags flags, ConsoleVariableFlags test)
    {
        return (static_cast<uint32>(flags) & static_cast<uint32>(test)) != 0;
    }

    /// 設定元優先度を取得（0-255、大きいほど高優先度）
    FORCEINLINE uint32 GetSetByPriority(ConsoleVariableFlags flags)
    {
        return (static_cast<uint32>(flags) >> 16) & 0xFF;
    }

    /// 設定元を比較（aがbより高優先度ならtrue）
    FORCEINLINE bool HasHigherPriority(ConsoleVariableFlags a, ConsoleVariableFlags b)
    {
        return GetSetByPriority(a) > GetSetByPriority(b);
    }

    /// 設定可能か判定（新しい設定元が現在より高優先度ならtrue）
    FORCEINLINE bool CanSetWithPriority(ConsoleVariableFlags currentFlags, ConsoleVariableFlags newSetBy)
    {
        return GetSetByPriority(newSetBy) >= GetSetByPriority(currentFlags);
    }

    // 前方宣言
    class IConsoleVariable;

    /// コンソール変数変更時のコールバック
    ///
    /// @param variable 変更された変数
    /// @note コールバックはSet()を呼び出したスレッドで同期的に実行される
    using ConsoleVariableDelegate = void(*)(IConsoleVariable* variable);

    /// コールバック登録ハンドル
    ///
    /// 登録解除に使用。0は無効なハンドル。
    using ConsoleVariableCallbackHandle = uint32;

    /// 無効なコールバックハンドル
    constexpr ConsoleVariableCallbackHandle kInvalidCallbackHandle = 0;

    /// コンソールオブジェクト基底インターフェース
    ///
    /// 変数とコマンドの共通基底。
    class IConsoleObject
    {
    public:
        virtual ~IConsoleObject() = default;

        /// ヘルプテキスト取得
        virtual const TCHAR* GetHelp() const = 0;

        /// ヘルプテキスト設定
        virtual void SetHelp(const TCHAR* help) = 0;

        /// フラグ取得
        virtual ConsoleVariableFlags GetFlags() const = 0;

        /// フラグ設定
        virtual void SetFlags(ConsoleVariableFlags flags) = 0;

        /// 変数として取得（変数でなければnullptr）
        virtual IConsoleVariable* AsVariable() { return nullptr; }
    };

    /// コンソール変数インターフェース
    ///
    /// 実行時に変更可能な設定値。
    /// int, float, bool, string型をサポート。
    ///
    /// ## スレッドセーフティ
    ///
    /// - **Get系メソッド**: スレッドセーフ（アトミック読み取り）
    /// - **Set系メソッド**: スレッドセーフ（内部ロック）
    /// - **RenderThreadSafeフラグ**: レンダースレッドからの読み取りを明示的に許可
    ///
    /// ## 設定優先度
    ///
    /// Set()のflagsパラメータで設定元を指定。
    /// 低優先度の設定元は高優先度の設定を上書きできない。
    /// 詳細はConsoleVariableFlagsを参照。
    ///
    /// ## 型変換
    ///
    /// 内部型と異なる型でGet/Setした場合:
    /// - int → float: 暗黙変換
    /// - float → int: 切り捨て
    /// - 数値 → bool: 0以外はtrue
    /// - 数値 ↔ string: 文字列パース/フォーマット
    class IConsoleVariable : public IConsoleObject
    {
    public:
        /// 整数値として取得
        /// @note スレッドセーフ
        virtual int32 GetInt() const = 0;

        /// 浮動小数点値として取得
        /// @note スレッドセーフ
        virtual float GetFloat() const = 0;

        /// ブール値として取得
        /// @note スレッドセーフ
        virtual bool GetBool() const = 0;

        /// 文字列として取得
        /// @return 内部バッファへのポインタ（次のSet呼び出しまで有効）
        /// @note スレッドセーフ（ただし返されたポインタは即座にコピーすること）
        virtual const TCHAR* GetString() const = 0;

        /// 整数値を設定
        ///
        /// @param value 新しい値
        /// @param flags 設定元フラグ（SetByXxx）
        /// @note 優先度が現在より低い場合は無視される
        /// @note スレッドセーフ
        virtual void Set(int32 value, ConsoleVariableFlags flags = ConsoleVariableFlags::SetByCode) = 0;

        /// 浮動小数点値を設定
        virtual void Set(float value, ConsoleVariableFlags flags = ConsoleVariableFlags::SetByCode) = 0;

        /// 文字列値を設定
        /// @param value 設定する文字列（コピーされる）
        virtual void Set(const TCHAR* value, ConsoleVariableFlags flags = ConsoleVariableFlags::SetByCode) = 0;

        /// 変更コールバック登録（単一、レガシー）
        ///
        /// @param callback 呼び出すコールバック（nullptrで解除）
        /// @note コールバックはSet呼び出しスレッドで同期実行される
        /// @note 複数登録時は上書きされる（後方互換用）
        /// @deprecated AddOnChangedCallback()を推奨
        virtual void SetOnChangedCallback(ConsoleVariableDelegate callback) = 0;

        /// 変更コールバック追加（複数登録対応）
        ///
        /// @param callback 呼び出すコールバック
        /// @return 登録ハンドル（解除に使用）、失敗時kInvalidCallbackHandle
        /// @note 同じコールバックを複数回登録可能
        virtual ConsoleVariableCallbackHandle AddOnChangedCallback(ConsoleVariableDelegate callback) = 0;

        /// 変更コールバック解除
        ///
        /// @param handle AddOnChangedCallbackで取得したハンドル
        /// @return 解除成功した場合true
        virtual bool RemoveOnChangedCallback(ConsoleVariableCallbackHandle handle) = 0;

        /// 全コールバック解除
        virtual void ClearOnChangedCallbacks() = 0;

        /// 現在の設定元を取得
        virtual ConsoleVariableFlags GetSetBy() const
        {
            return GetFlags() & ConsoleVariableFlags::SetByMask;
        }

        /// デフォルト値にリセット
        virtual void Reset() = 0;

        IConsoleVariable* AsVariable() override { return this; }
    };

    /// コンソールコマンドインターフェース
    ///
    /// 引数付きで実行可能なコマンド。
    class IConsoleCommand : public IConsoleObject
    {
    public:
        /// コマンド実行
        /// @param args 引数文字列
        /// @return 実行成功したか
        virtual bool Execute(const TCHAR* args) = 0;
    };
}
```

## 検証

- `ConsoleVariableFlags` のビット演算が正しく動作
- `IConsoleVariable` が抽象インターフェースとして機能
- `HasFlag` ヘルパーが正しく判定

## 次のサブ計画

→ 08-03b: ConsoleManager実装
