...
void SpriteBatch::Begin() {
    ...
    // 現在のレンダーターゲットサイズから正射影行列を自動設定
    Texture* backBuffer = Renderer::Get().GetBackBuffer();
    if (backBuffer) {
        float width = static_cast<float>(backBuffer->Width());
        float height = static_cast<float>(backBuffer->Height());
        // 左上(0,0)、右下(width, height)の2D座標系（左手系）
        // HLSLはcolumn-majorなので転置が必要
        Matrix ortho = LH::CreateOrthographicOffCenter(
            0.0f, width, height, 0.0f, 0.0f, 1.0f);
...
void SpriteBatch::Begin(const Matrix& customViewProjection) {
    if (!initialized_) {
        LOG_WARN("SpriteBatch: 初期化されていません");
        return;
    }
    ...
    cbufferData_.viewProjection = customViewProjection;
...
void SpriteBatch::FlushBatch() {
    ...
    // 頂点データを一括でマップして書き込み（Map/Unmapは1回だけ）
    ...
    ctx.UnmapBuffer(vertexBuffer_.get());  // 一度だけUnmap

    // バッチ描画（Map/Unmapなし、テクスチャ変更時にDrawIndexedのみ）
