...
public:
    ...
    //! @brief バッチ描画開始（自動正射影）
    //!
    //! 現在のレンダーターゲットサイズから2D用正射影行列を自動設定。
    //! 座標系: 左上(0,0)、右下(width, height)
    //------------------------------------------------------------------------
    void Begin();

    //------------------------------------------------------------------------
    //! @brief バッチ描画開始（カスタム行列）
    //! @param customViewProjection カスタムビュープロジェクション行列（転置済み）
    //!
    //! 3Dカメラ追従UIなど特殊なケースで使用。
    //! HLSLがcolumn-majorのため、呼び出し側で転置すること。
    //------------------------------------------------------------------------
    void Begin(const Matrix& customViewProjection);
