...
namespace NS
{
    namespace RHI
    {
        ...
        enum class EShaderStageFlags : uint32
        {
            ...
            AllRayTracing = RayGen | RayMiss | RayClosestHit | RayAnyHit | RayIntersection | RayCallable,
            All = 0xFFFFFFFF,
        };
        ...
        {
            return type == ERHIDescriptorHeapType::CBV_SRV_UAV || type == ERHIDescriptorHeapType::Sampler;
        }
        ...
        {
            return EnumHasAnyFlags(usage, ERHIBufferUsage::VertexBuffer | ERHIBufferUsage::IndexBuffer);
        }
        ...
        {
            return EnumHasAnyFlags(usage, ERHIBufferUsage::StructuredBuffer | ERHIBufferUsage::ByteAddressBuffer);
        }
        ...
        {
            return dim == ERHITextureDimension::Texture2DMS || dim == ERHITextureDimension::Texture2DMSArray;
        }

        /// キューブマップか
        inline bool IsCubeTexture(ERHITextureDimension dim)
        {
            return dim == ERHITextureDimension::TextureCube || dim == ERHITextureDimension::TextureCubeArray;
        }
        ...
        struct RHIComponentMapping
        {
            ...
            /// 全チャンネルを指定値に
            static constexpr RHIComponentMapping All(ERHIComponentSwizzle s) { return RHIComponentMapping{s, s, s, s}; }
        };
        ...
        /// クエリフラグ（IRHIDevice.hから参照されるためここで定義）
        enum class ERHIQueryFlags : uint32
        {
            None = 0,
            Result64Bit = 1 << 0,      ///< 結果が64bit
            WithAvailability = 1 << 1, ///< 利用可能フラグを含む
            Wait = 1 << 2,             ///< 結果待ち
            PartialResults = 1 << 3,   ///< 部分的な結果を許可
        };
        RHI_ENUM_CLASS_FLAGS(ERHIQueryFlags)

        //=========================================================================
        // ERHIHeapType: ヒープタイプ（IRHITexture.hから参照されるためここで定義）
        //=========================================================================
