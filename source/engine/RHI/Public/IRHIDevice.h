...
namespace NS
{
    namespace RHI
    {
        ...
        /// RHIGPUEvent.h の型（ヘッダー依存を避けるため宣言のみ）
        enum class ERHIProfilerType : uint8;
        using RHIDeviceLostCallback = void (*)(const RHIGPUCrashInfo&, void*);
        ...
        public:
            ...
            /// コマンドアロケーター解放
            virtual void ReleaseCommandAllocator(IRHICommandAllocator* allocator,
                                                 IRHIFence* fence,
                                                 uint64 fenceValue) = 0;
            ...
            /// Bindless GPU可視ヒープ取得（SetDescriptorHeaps用）
            virtual IRHIDescriptorHeap* GetBindlessSRVUAVHeap() const { return nullptr; }
            virtual IRHIDescriptorHeap* GetBindlessSamplerHeap() const { return nullptr; }
