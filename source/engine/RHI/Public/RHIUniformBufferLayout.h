...
namespace NS::RHI
{
    ...
    //=========================================================================
    // RHIUniformBufferLayout (24-01)
    //=========================================================================

    /// UniformBufferレイアウト
    /// 一度構築したら不変
    class RHI_API RHIUniformBufferLayout
    {
    public:
    ...
    //=========================================================================
    // RHIUniformBufferLayoutBuilder (24-01)
    //=========================================================================

    /// UniformBufferレイアウトビルダー
    /// HLSL cbufferルールに従ってオフセットを自動計算
    class RHI_API RHIUniformBufferLayoutBuilder
    {
    public:
        ...
        /// int追加
        RHIUniformBufferLayoutBuilder& AddInt(const char* name) { return AddElement(name, ERHIUniformType::Int, 4, 4); }
    ...
    private:
        RHIUniformBufferLayoutBuilder& AddElement(const char* name,
    ...
    //=========================================================================
    // TRHIUniformBuffer (24-01)
    //=========================================================================

    /// 型付きUniformBuffer
    /// C++構造体とGPUレイアウトの一致を保証
    template <typename T> class TRHIUniformBuffer
    {
    public:
        static_assert(sizeof(T) % 16 == 0, "Uniform buffer size must be 16-byte aligned");

        explicit TRHIUniformBuffer(RHIUniformBufferLayoutRef layout) : m_layout(std::move(layout)) {}
