...
namespace NS::result::detail
{
    ...
    /// Result型の内部表現とビット操作
    /// 32bitレイアウト:
    ///   bit 0-8   (9bit):  Module (1-511)
    ///   bit 9-21  (13bit): Description (0-8191)
    ///   bit 22-23 (2bit):  Persistence (0=Unknown, 1=Transient, 2=Permanent)
    ///   bit 24-25 (2bit):  Severity (0=Unknown, 1=Recoverable, 2=Fatal)
    ///   bit 26-31 (6bit):  将来拡張用
    class ResultTraits
    {
    public:
        ...
        static constexpr InnerType kModuleMask = ((InnerType(1) << kModuleBitsCount) - 1) << kModuleBitsOffset;
        ...
        static constexpr InnerType kDescriptionMask = ((InnerType(1) << kDescriptionBitsCount) - 1)
                                                      << kDescriptionBitsOffset;
        ...
        static constexpr InnerType kReservedMask = ((InnerType(1) << kReservedBitsCount) - 1) << kReservedBitsOffset;

        // Persistence: 2bit (22-23) within Reserved
        static constexpr int kPersistenceBitsOffset = kReservedBitsOffset;
        static constexpr int kPersistenceBitsCount = 2;
        static constexpr InnerType kPersistenceMask = ((InnerType(1) << kPersistenceBitsCount) - 1)
                                                      << kPersistenceBitsOffset;

        // Severity: 2bit (24-25) within Reserved
        ...
        static constexpr InnerType kSeverityMask = ((InnerType(1) << kSeverityBitsCount) - 1) << kSeverityBitsOffset;

        // Future: 6bit (26-31) within Reserved
        static constexpr int kFutureBitsOffset = kSeverityBitsOffset + kSeverityBitsCount;
        static constexpr int kFutureBitsCount = 6;

        /// コンパイル時値生成
        template <int Module, int Description>
        struct MakeInnerValueStatic
            : std::integral_constant<InnerType,
                                         ...
                                         (static_cast<InnerType>(Description) << kDescriptionBitsOffset)>
        {
            ...
            static_assert(kDescriptionBegin <= Description && Description < kDescriptionEnd,
        ...
        [[nodiscard]] static constexpr InnerType MakeInnerValueWithReserved(int module,
                                                                            int description,
                                                                            int reserved) noexcept
        {
            return MakeInnerValue(module, description) | (static_cast<InnerType>(reserved) << kReservedBitsOffset);
        }
        ...
        {
            return (value & ~kReservedMask) | (static_cast<InnerType>(reserved) << kReservedBitsOffset);
        }
        ...
        {
            return (value & ~kPersistenceMask) | (static_cast<InnerType>(persistence) << kPersistenceBitsOffset);
        }

        [[nodiscard]] static constexpr InnerType SetSeverity(InnerType value, int severity) noexcept
        {
            return (value & ~kSeverityMask) | (static_cast<InnerType>(severity) << kSeverityBitsOffset);
        }
    ...
    private:
        static_assert(kModuleBitsCount + kDescriptionBitsCount + kReservedBitsCount == sizeof(InnerType) * CHAR_BIT,
                      "Total bits must equal 32");
        static_assert(kPersistenceBitsCount + kSeverityBitsCount + kFutureBitsCount == kReservedBitsCount,
